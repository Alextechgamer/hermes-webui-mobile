# Grok on Mac — archive IPA onto the existing v0.9.22 tag

Paste everything below the line into Grok on the Apple Silicon Mac.

---

You are on Apple Silicon macOS with Xcode. Your only job is to archive the **native SwiftUI** Hermes WebUI app and **attach the IPA to the existing GitHub release `v0.9.22`**. Linux already shipped the APK. You cannot compile this on Linux; do not try to rebuild Android.

Repo: https://github.com/Alextechgamer/hermes-webui-mobile
Tag already exists: `v0.9.22` on `main`. Release already has `hermes-webui-0.9.22-debug.apk`.
...
**Do not touch `v0.9.12`–`v0.9.21`.**

This is a QA/bugfix release from a full on-device test pass: CLI tab filters client-side via is_cli_session, chat stream detaches on session switch (no bleed into new chats), the send button is pinned outside the scrolling chip row, sidebar keeps server recency order (pinned lifted, order-stable), and deleting the current session falls back to the next one so Files/Terminal keep a valid session_id. OfficialDashView stays deleted. Insights stays `/api/insights`.
...
2. `git clone` if needed, else `git fetch && git checkout main && git pull --ff-only`. Confirm:
   - `ios/project.yml` has `MARKETING_VERSION: "0.9.22"` and `CURRENT_PROJECT_VERSION: "34"`.
...
- Rename the exported IPA to **`hermes-webui-0.9.22.ipa`**
...
cp "$PWD/build/ipa/"*.ipa "$PWD/build/ipa/hermes-webui-0.9.22.ipa"
```
...
```bash
gh auth status
gh release view v0.9.22
gh release upload v0.9.22 hermes-webui-0.9.22.ipa
```

Use the real path to the IPA. **Never** `--clobber` the APK. If `hermes-webui-0.9.22.ipa` is already on the release, stop and report.

```bash
gh release view v0.9.22 --json assets --jq '.assets[] | {name,size}'
```

You should see both `hermes-webui-0.9.22-debug.apk` and `hermes-webui-0.9.22.ipa`. Reply with the release URL and both asset names + sizes. Done.
