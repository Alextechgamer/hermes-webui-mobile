# Grok on Mac — archive IPA onto the existing v0.9.19 tag

Paste everything below the line into Grok on the Apple Silicon Mac.

---

You are on Apple Silicon macOS with Xcode. Your only job is to archive the **native SwiftUI** Hermes WebUI app and **attach the IPA to the existing GitHub release `v0.9.19`**. Linux already shipped the APK. You cannot compile this on Linux; do not try to rebuild Android.

Repo: https://github.com/Alextechgamer/hermes-webui-mobile
Tag already exists: `v0.9.19` on `main`. Release already has `hermes-webui-0.9.19-debug.apk`.
...
**Do not touch `v0.9.12`–`v0.9.18`.**

This release adds leftover WebUI 1:1 features: Files create/rename/delete, slash-command autocomplete + exec, session retry/undo/regenerate title, YOLO toggle, Settings sign-out and updates check/apply. OfficialDashView stays deleted. Insights stays `/api/insights`.
...
2. `git clone` if needed, else `git fetch && git checkout main && git pull --ff-only`. Confirm:
   - `ios/project.yml` has `MARKETING_VERSION: "0.9.19"` and `CURRENT_PROJECT_VERSION: "31"`.
...
- Rename the exported IPA to **`hermes-webui-0.9.19.ipa`**
...
cp "$PWD/build/ipa/"*.ipa "$PWD/build/ipa/hermes-webui-0.9.19.ipa"
```
...
```bash
gh auth status
gh release view v0.9.19
gh release upload v0.9.19 hermes-webui-0.9.19.ipa
```

Use the real path to the IPA. **Never** `--clobber` the APK. If `hermes-webui-0.9.19.ipa` is already on the release, stop and report.

```bash
gh release view v0.9.19 --json assets --jq '.assets[] | {name,size}'
```

You should see both `hermes-webui-0.9.19-debug.apk` and `hermes-webui-0.9.19.ipa`. Reply with the release URL and both asset names + sizes. Done.
